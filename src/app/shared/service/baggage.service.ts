...
export class BaggageService {
  ...
  delete(id) {
    // tslint:disable-next-line:max-line-length
    return this.http.delete(ApiConstants.baseURl + `/lockers/${id.locationId}/${id.number}`, { headers: { Authorization: `Bearer ${localStorage.getItem('access-token')}` } })
      .pipe(map(res => {
        return {
          status: res['result']
        };
  ...
  // locker
  getLocker(id) {
    return this.httpService.get(ApiConstants.baseURl + `/lockers/getlocker/${id}`).pipe(map(res => {
      console.log(res);
      return {
        status: res['result'],
        data: res['data']
